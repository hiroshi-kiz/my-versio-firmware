...
> **既知の問題（保留中）**: D（Fold）とG（8vize）のどちらか一方だけが機能し、もう一方は無反応になる。しかも**どちらが機能する側になるかが時間とともに入れ替わる**（配線を触っていないのに、前回はDが正常でGが無反応だったのが、後日試すとGが正常でDが無反応になった）。
>
> 調査の結果:
> - Noise Engineering純正ファームウェア(2種類)では両方とも問題なく動作する → **ハードウェアの実装自体は正常**
> - Daisy Seedの全ADC対応ピン(16本)をスキャンしても、無反応側のノブに反応するピンは他に見つからなかった
> - コード上でD/Gの割り当てチャンネルを入れ替えても、無反応/クロストークの症状は解消せず、別の場所（B）に移っただけだった
> - 固定的な配線ミスなら毎回同じ側が無反応になるはずだが、時間とともに入れ替わることから、**特定チャンネル間の恒常的なクロストークというより、断続的な接触不良（半田・コネクタの緩み等）の可能性が高い**と判断
> - コネクタの差し直し・目視確認では改善しなかった
>
> ソフトウェア側でこれ以上安定して切り分ける手段がないため、**現状動いている方のノブ（DまたはG、その時々で変わる）を使う**という運用で割り切っている。コード自体は`KNOB_2`を読む実装のままで変更していない。
...
| G | 8vize | 本来は未割当。**Dが無反応の時は代わりにこちらがMod Speedとして機能することがある、上記参照** | `KNOB_1`(コード上は未使用) |
...
検出には`kClockThresholdHigh`(0.35)/`kClockThresholdLow`(0.15)の2段階のヒステリシス（シュミットトリガー）を使っている。単一のしきい値だと信号のノイズ/リンギングで1パルスに対して何度も誤検出してしまい、テンポが同期しなかったため。

DELAYタイム切替時のグライドタイムは`kFixedGlideTimeSec`（`MyVersioFirmware.cpp`、現在0.08秒）で固定。

### トグルスイッチ

- **T1**（3ポジション）: DELAYタイムの分周比（4分 / 付点8分 / 8分音符）
...
Bでモジュレーション量（深さ）、Dでモジュレーション速度（Envelope時はディケイタイムとして働く）を調整する。LFO(Saw Down/Square/Triangle)の位相はトリガーのたびに0へリセットされるため、毎回同じ挙動で鳴る（リズム楽器として安定する）。Sample & Hold/Noiseは仕様上ランダムな値を使うため、トリガーごとに異なる結果になるのは意図通り。

### Clap（旧Noise）

T2をClapにすると、OSCがホワイトノイズ→ハイパス→ローパス（レゾナンス無しを直列、808寄りの柔らかい帯域制限ノイズ）を通った音になる。トリガー直後に短いノイズバーストを3回連打（808/909系ハンドクラップの模倣、角を丸めてクリック感を軽減）し、その後は通常通りE（AMPディケイ）でテールが減衰する。フィルター通過で下がる音量を`kClapGain`（`MyVersioFirmware.cpp`、現在3.0倍）で補正している。
